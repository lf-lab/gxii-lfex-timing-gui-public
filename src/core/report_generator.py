...
from src.core.waveform_library import WaveformLibrary
...
class ReportGenerator:
    ...
    def generate_single_page_report(self, app_instance, report_author: str, sweep_speed_setting: str,
                                    laser_comments: str, app_version: str, hide_lfex_2nd_peak_detected: bool) -> Optional[bytes]:
        ...
        try:
            ...
            shot_datetime = results.get('shot_datetime', '')
            
            # プロットを生成し、バイトデータとして取得
            plot_bytes_list = self.plot_manager.generate_report_plots(results, self.waveform_library, hide_lfex_2nd_peak_detected)
            ...
            draw = ImageDraw.Draw(report_image) # drawオブジェクトの初期化をここで行う
...
def get_report_generator() -> 'ReportGenerator':
    """
    セッション状態からReportGeneratorインスタンスを取得する。
    インスタンスはGXIILFEXAppで初期化されていることを前提とする。
    """
    if 'report_generator' not in st.session_state:
        # GXIILFEXAppで初期化されるはずなので、通常ここには到達しない
        log_warning("ReportGeneratorが初期化されていません。フォールバックします。", "report_generator")
        
        # アプリケーションインスタンスからwaveform_libraryを取得しようと試みる
        if 'app_instance' in st.session_state and hasattr(st.session_state.app_instance, 'waveform_library'):
            waveform_library = st.session_state.app_instance.waveform_library
            log_info("app_instanceからwaveform_libraryを取得しました。", "report_generator")
        else:
            log_warning("app_instanceからwaveform_libraryを取得できませんでした。新しいWaveformLibraryを作成します。", "report_generator")
            waveform_library = WaveformLibrary() # フォールバック
            
        st.session_state.report_generator = ReportGenerator(waveform_library=waveform_library)
        
    # 常に最新のwaveform_libraryを参照するように更新
    elif 'app_instance' in st.session_state and hasattr(st.session_state.app_instance, 'waveform_library'):
        st.session_state.report_generator.waveform_library = st.session_state.app_instance.waveform_library
